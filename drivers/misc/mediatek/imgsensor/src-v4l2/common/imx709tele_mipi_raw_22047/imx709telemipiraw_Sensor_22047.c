...
static void write_frame_len(struct subdrv_ctx *ctx, kal_uint32 fll, kal_uint32 exp_cnt_value)
{
    ...
    LOG_DEBUG("fll %d exp_cnt %d, extend %d, fastmode %d\n", ctx->frame_length, exp_cnt,
...
{
    LOG_DEBUG("dummyline = %d, dummypixels = %d, frame_length:%d, line_length:%d\n",
...
{
    LOG_DEBUG("Before shutters[0] =%d, framelength =%d/%d\n",
    ...
    LOG_DEBUG("Exit shutters[0] =%d, framelength =%d/%d\n",
        shutters[0], ctx->frame_length, frame_length);
        ...
        enum SENSOR_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    LOG_DEBUG("scenario_id = %d, ctx->current_scenario_id(%d), framerate = %d\n", scenario_id, ctx->current_scenario_id, framerate);
