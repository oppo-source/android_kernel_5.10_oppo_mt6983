...
static DEFINE_MUTEX(gimx709tele_eeprom_mutex);

bool read_imx709tele_eeprom_22047(struct subdrv_ctx *ctx, kal_uint16 addr,
                   BYTE *data, int size)
{
    if (addr + size > IMX709TELE_MAX_OFFSET) {
        return false;
    }

    if (adaptor_i2c_rd_p8(ctx->i2c_client,
            IMX709TELE_EEPROM_SLAVE_ADDRESS >> 1, addr, data, size) < 0) {
        return false;
    }
...
unsigned int read_imx709tele_LRC_22047(struct subdrv_ctx *ctx, kal_uint16 *data)
{
    ...
    static BYTE imx709tele_LRC_data[LRC_SIZE] = { 0 };
    ...
    LOG_INF("read imx709tele LRC, otp_offset = %d, size = %u\n",
        pinfo->LRC_addr, pinfo->LRC_size);

    mutex_lock(&gimx709tele_eeprom_mutex);
    if ((readed_size == 0) &&
        read_imx709tele_eeprom_22047(ctx, pinfo->LRC_addr, imx709tele_LRC_data,
                   pinfo->LRC_size)) {
        readed_size = pinfo->LRC_size;
    }
    mutex_unlock(&gimx709tele_eeprom_mutex);
    ...
    for (idx = 0; idx < LRC_SIZE; idx++) {
        ...
        data[2 * idx + 1] = imx709tele_LRC_data[idx];
    }
    ...
    read_imx709tele_eeprom_22047(ctx, pinfo->LRC_addr + pinfo->LRC_size, &lrc_flag, 1);
...
unsigned int read_imx709tele_QSC_22047(struct subdrv_ctx *ctx, kal_uint8 *data)
{
    ...
    read_imx709tele_eeprom_22047(ctx, OTP_QSC_OFFSET, data, QSC_SIZE);

    for (idx = 0; idx < QSC_SIZE; idx++) {
        LOG_INF("alex qsc data imx709tele_QSC_setting[0x%x] = 0x%x", idx, data[idx]);
    }

    read_imx709tele_eeprom_22047(ctx, 0x2A32, &qsc_ver, 1);
    LOG_INF("QSC Version: 0x%x", qsc_ver);
    return 0;
}

struct eeprom_map_info imx709tele_eeprom_info_22047[] = {
    ...
    { EEPROM_META_STEREO_MW_MAIN_DATA, 0x0000, 0x0000, 0x0000, 0, false },
                     ...
                     BYTE *data, int size)
{
    ...
    if (meta_id != imx709tele_eeprom_info_22047[meta_id].meta)
        return -1;

    if (size != imx709tele_eeprom_info_22047[meta_id].size)
        return -1;

    addr = imx709tele_eeprom_info_22047[meta_id].start;
    readsize = imx709tele_eeprom_info_22047[meta_id].size;

    read_imx709tele_eeprom_22047(ctx, addr, data, readsize);
