...
#include "omegac2front_vcm_23081.h"

#define OMEGAC2FRONT_VCM_SLAVE_ADDRESS 0x18
...
static bool selective_write_vcm(struct subdrv_ctx *ctx, struct regval_list *vals, u32 len)
{
    ...
    for (i = 0; i < len; i++) {
        ret = adaptor_i2c_wr_u8_u8(ctx->i2c_client, OMEGAC2FRONT_VCM_SLAVE_ADDRESS >> 1,
...
unsigned int write_omegac2front_vcm_23081_init(struct subdrv_ctx *ctx)
{
