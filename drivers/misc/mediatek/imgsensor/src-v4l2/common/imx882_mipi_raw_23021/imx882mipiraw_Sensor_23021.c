...
kal_uint32 align_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
...
kal_uint32 align_framelength(struct subdrv_ctx *ctx, kal_uint32 framelength)
{
...
static void set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate, kal_bool min_framelength_en)
{
    ...
    if (frame_length >= ctx->min_frame_length) {
        ...
        ctx->frame_length = align_framelength(ctx, ctx->frame_length);
    }
    else
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter, kal_bool gph)
{
    ...
    shutter = align_shutter(ctx, shutter);
                ...
                kal_uint16 frame_length)
{
    ...
    for (i = 0; i < shutter_cnt; i++) {
        ...
        shutters[i] = align_shutter(ctx, shutters[i] / shutter_cnt) * shutter_cnt;
    }
        ...
        kal_uint32 le, kal_uint32 me, kal_uint32 se, kal_bool gph)
{
    ...
    if (le) {
        ...
        le = align_shutter(ctx, le / exposure_cnt) * exposure_cnt;
    }
    if (me) {
        me = (kal_uint16)max(imgsensor_info.min_shutter, (kal_uint32)me);
        me = align_shutter(ctx, me / exposure_cnt) * exposure_cnt;
    }
    if (se) {
        se = (kal_uint16)max(imgsensor_info.min_shutter, (kal_uint32)se);
        se = align_shutter(ctx, se / exposure_cnt) * exposure_cnt;
    }
        ...
        enum SENSOR_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    ctx->frame_length = align_framelength(ctx, ctx->frame_length);
    ctx->min_frame_length = align_framelength(ctx, ctx->min_frame_length);
    set_dummy(ctx);
