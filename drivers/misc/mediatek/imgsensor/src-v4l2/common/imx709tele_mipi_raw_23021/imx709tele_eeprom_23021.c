...
unsigned int read_imx709tele_LRC_23021(struct subdrv_ctx *ctx, kal_uint16 *data)
{
    ...
    if ((readed_size == 0) &&
        read_imx709tele_eeprom_23021(ctx, pinfo->LRC_addr, imx709tele_LRC_data,
                   pinfo->LRC_size)) {
    ...
    read_imx709tele_eeprom_23021(ctx, pinfo->LRC_addr + pinfo->LRC_size, &lrc_flag, 1);
...
unsigned int read_imx709tele_QSC_23021(struct subdrv_ctx *ctx, kal_uint8 *data)
{
    ...
    read_imx709tele_eeprom_23021(ctx, OTP_QSC_OFFSET, data, QSC_SIZE);

    for (idx = 0; idx < QSC_SIZE; idx++) {
        LOG_INF("alex qsc data imx709tele_QSC_setting[0x%x] = 0x%x", idx, data[idx]);
    }

    read_imx709tele_eeprom_23021(ctx, 0x2A32, &qsc_ver, 1);
    LOG_INF("QSC Version: 0x%x", qsc_ver);
    return 0;
}

struct eeprom_map_info imx709tele_eeprom_info_23021[] = {
                     ...
                     BYTE *data, int size)
{
    ...
    if (meta_id != imx709tele_eeprom_info_23021[meta_id].meta)
        return -1;

    if (size != imx709tele_eeprom_info_23021[meta_id].size)
        return -1;

    addr = imx709tele_eeprom_info_23021[meta_id].start;
    readsize = imx709tele_eeprom_info_23021[meta_id].size;

    read_imx709tele_eeprom_23021(ctx, addr, data, readsize);
