 ...
 *
 ****************************************************************************/
#define PFX "IMX766_pdafotp_21121"
...
#define IMX766_EEPROM_READ_ID_21121  0xA3
#define IMX766_EEPROM_WRITE_ID_21121 0xA2
#define IMX766_I2C_SPEED_21121       100
#define IMX766_MAX_OFFSET_21121      0x8000
...
static DEFINE_MUTEX(gimx766_eeprom_mutex_21121);

static bool selective_read_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data)
{
    if (addr > IMX766_MAX_OFFSET_21121)
        return false;

    if (adaptor_i2c_rd_u8(ctx->i2c_client,
            IMX766_EEPROM_READ_ID_21121 >> 1, addr, data) < 0) {
        return false;
    }
    return true;
}

static bool read_imx766_eeprom_21121(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size)
{
    int i = 0;
    int offset = addr;
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
    ...
    mutex_lock(&gimx766_eeprom_mutex_21121);
    if (pinfo == NULL) {
        read_imx766_eeprom_21121(ctx, 0x1, read_data, 4);
        if (((read_data[3] << 24) |
             (read_data[2] << 16) |
             (read_data[1] << 8) |
    ...
    mutex_unlock(&gimx766_eeprom_mutex_21121);
...
{
    static BYTE IMX766_LRC_data_21121[LRC_SIZE] = { 0 };
    ...
    mutex_lock(&gimx766_eeprom_mutex_21121);
    if ((readed_size == 0) &&
        read_imx766_eeprom_21121(ctx, pinfo->LRC_addr,
                   IMX766_LRC_data_21121, pinfo->LRC_size)) {
        readed_size = pinfo->LRC_size;
    }
    mutex_unlock(&gimx766_eeprom_mutex_21121);

    memcpy(data, IMX766_LRC_data_21121, pinfo->LRC_size);
    return readed_size;
}
...
unsigned int read_imx766_DCC_21121(struct subdrv_ctx *ctx, BYTE *data)
{
    ...
    mutex_lock(&gimx766_eeprom_mutex_21121);
    if ((readed_size == 0) &&
        read_imx766_eeprom_21121(ctx, pinfo->DCC_addr,
...
struct eeprom_map_info imx766_eeprom_info_21121[] = {
                     ...
                     BYTE *data, int size)
{
    ...
    if (meta_id != imx766_eeprom_info_21121[meta_id].meta)
        return -1;

    if (size != imx766_eeprom_info_21121[meta_id].size)
        return -1;

    addr = imx766_eeprom_info_21121[meta_id].start;
    readsize = imx766_eeprom_info_21121[meta_id].size;

    read_imx766_eeprom_21121(ctx, addr, data, readsize);
