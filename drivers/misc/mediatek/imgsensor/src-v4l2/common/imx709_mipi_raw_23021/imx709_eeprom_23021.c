...
bool read_imx709_eeprom_23021(struct subdrv_ctx *ctx, kal_uint16 addr,
                   BYTE *data, int size)
{
...
unsigned int read_imx709_LRC_23021(struct subdrv_ctx *ctx, kal_uint16 *data)
{
    ...
    if ((readed_size == 0) &&
        read_imx709_eeprom_23021(ctx, pinfo->LRC_addr, imx709_LRC_data,
                   pinfo->LRC_size)) {
    ...
    read_imx709_eeprom_23021(ctx, pinfo->LRC_addr + pinfo->LRC_size, &lrc_flag, 1);
    LOG_INF("LRC flag0x%x[1:valid, other:Invalid]", lrc_flag);
    return readed_size;
}

unsigned int read_imx709_QSC_23021(struct subdrv_ctx *ctx, kal_uint8 *data)
{
    kal_uint16 idx = 0;
    kal_uint8 qsc_ver = 0;

    read_imx709_eeprom_23021(ctx, OTP_QSC_OFFSET, data, QSC_SIZE);

    for (idx = 0; idx < QSC_SIZE; idx++) {
        LOG_INF("alex qsc data imx709_QSC_setting[0x%x] = 0x%x", idx, data[idx]);
    }

    read_imx709_eeprom_23021(ctx, 0x2A32, &qsc_ver, 1);
    LOG_INF("QSC Version: 0x%x", qsc_ver);
    return 0;
}

struct eeprom_map_info imx709_eeprom_info_23021[] = {
...
unsigned int read_imx709_eeprom_info_23021(struct subdrv_ctx *ctx, kal_uint16 meta_id,
                     BYTE *data, int size)
{
    kal_uint16 addr;
    int readsize;

    if (meta_id != imx709_eeprom_info_23021[meta_id].meta)
        return -1;

    if (size != imx709_eeprom_info_23021[meta_id].size)
        return -1;

    addr = imx709_eeprom_info_23021[meta_id].start;
    readsize = imx709_eeprom_info_23021[meta_id].size;

    read_imx709_eeprom_23021(ctx, addr, data, readsize);
