 ...
 *
 ****************************************************************************/
#define PFX "IMX890_pdafotp_22017"
...
#define IMX890_EEPROM_READ_ID_22017  0xA3
#define IMX890_EEPROM_WRITE_ID_22017 0xA2
#define IMX890_I2C_SPEED_22017       100
#define IMX890_MAX_OFFSET_22017      0x8000
...
static DEFINE_MUTEX(gimx890_eeprom_mutex_22017);

static bool selective_read_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data)
{
    if (addr > IMX890_MAX_OFFSET_22017)
        return false;

    if (adaptor_i2c_rd_u8(ctx->i2c_client,
            IMX890_EEPROM_READ_ID_22017 >> 1, addr, data) < 0) {
        return false;
    }
    return true;
}

static bool read_imx890_eeprom_22017(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size)
{
    int i = 0;
    int offset = addr;
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
    ...
    mutex_lock(&gimx890_eeprom_mutex_22017);
    if (pinfo == NULL) {
        read_imx890_eeprom_22017(ctx, 0x1, read_data, 4);
        if (((read_data[3] << 24) |
             (read_data[2] << 16) |
             (read_data[1] << 8) |
    ...
    mutex_unlock(&gimx890_eeprom_mutex_22017);

    return pinfo;
}

unsigned int read_imx890_LRC_22017(struct subdrv_ctx *ctx, BYTE *data)
{
    static BYTE IMX890_LRC_data_22017[LRC_SIZE] = { 0 };
    ...
    mutex_lock(&gimx890_eeprom_mutex_22017);
    if ((readed_size == 0) &&
        read_imx890_eeprom_22017(ctx, pinfo->LRC_addr,
                   IMX890_LRC_data_22017, pinfo->LRC_size)) {
        readed_size = pinfo->LRC_size;
    }
    mutex_unlock(&gimx890_eeprom_mutex_22017);

    memcpy(data, IMX890_LRC_data_22017, pinfo->LRC_size);
    return readed_size;
}

unsigned int read_imx890_DCC_22017(struct subdrv_ctx *ctx, BYTE *data)
{
    ...
    mutex_lock(&gimx890_eeprom_mutex_22017);
    if ((readed_size == 0) &&
        read_imx890_eeprom_22017(ctx, pinfo->DCC_addr,
...
struct eeprom_map_info imx890_eeprom_info_22017[] = {
...
unsigned int read_imx890_eeprom_info_22017(struct subdrv_ctx *ctx, kal_uint16 meta_id,
                     BYTE *data, int size)
{
    kal_uint16 addr;
    int readsize;

    if (meta_id != imx890_eeprom_info_22017[meta_id].meta)
        return -1;

    if (size != imx890_eeprom_info_22017[meta_id].size)
        return -1;

    addr = imx890_eeprom_info_22017[meta_id].start;
    readsize = imx890_eeprom_info_22017[meta_id].size;

    read_imx890_eeprom_22017(ctx, addr, data, readsize);
