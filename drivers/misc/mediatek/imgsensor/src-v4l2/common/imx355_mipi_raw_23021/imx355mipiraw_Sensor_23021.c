...
#include "imx355_ana_gain_table_23021.h"
#include "imx355mipiraw_Sensor_23021.h"
...
#define PFX "IMX355_camera_sensor_23021"
...
static struct oplus_get_eeprom_common_data imx355_common_data = {0};
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX355_SENSOR_ID_23021,
...
{
    LOG_INF("E %s.\n", __func__);
    imx355_table_write_cmos_sensor(ctx, custom7_setting_array,
            ...
            ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == IMX355_SENSOR_ID_23021) && (data_length == CALI_DATA_SLAVE_LENGTH)
	    ...
	    } else if ((pStereodata->uSensorId == IMX355_SENSOR_ID_23021) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == IMX355_AESYNC_START_ADDR_23021)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+1),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+2),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+3),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+4),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+5),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+6),
                    read_cmos_eeprom_8(ctx, IMX355_AESYNC_START_ADDR_23021+7));
...
static void read_module_data(struct subdrv_ctx *ctx)
{
	...
	memset(&imx355_common_data, 0x00, sizeof(imx355_common_data));

	imx355_common_data.header[EEPROM_MODULE_ID] = 2;
	read_imx355_eeprom_info_23021(ctx, EEPROM_META_MODULE_ID,
				&(imx355_common_data.data[idx]), 2);
	imgsensor_info.module_id = (kal_uint16)(imx355_common_data.data[idx + 1] << 8) |
				   imx355_common_data.data[idx];
	idx += imx355_common_data.header[EEPROM_MODULE_ID];

	imx355_common_data.header[EEPROM_SENSOR_ID] = 2;
	read_imx355_eeprom_info_23021(ctx, EEPROM_META_SENSOR_ID,
				&(imx355_common_data.data[idx]), 2);
	idx += imx355_common_data.header[EEPROM_SENSOR_ID];

	imx355_common_data.header[EEPROM_LENS_ID] = 2;
	read_imx355_eeprom_info_23021(ctx, EEPROM_META_LENS_ID,
				&(imx355_common_data.data[idx]), 2);
	idx += imx355_common_data.header[EEPROM_LENS_ID];

	imx355_common_data.header[EEPROM_VCM_ID] = 2;
	read_imx355_eeprom_info_23021(ctx, EEPROM_META_VCM_ID,
				&(imx355_common_data.data[idx]), 2);
	idx += imx355_common_data.header[EEPROM_VCM_ID];

	imx355_common_data.header[EEPROM_MODULE_SN] = 17;
	read_imx355_eeprom_info_23021(ctx, EEPROM_META_MODULE_SN,
				&(imx355_common_data.data[idx]), 17);
	idx += imx355_common_data.header[EEPROM_MODULE_SN];

    for (idx = 0; idx < 32; idx = idx + 4)
        LOG_INF("In %s:common data: %02x %02x %02x %02x\n", __func__,
               imx355_common_data.data[idx], imx355_common_data.data[idx + 1],
               imx355_common_data.data[idx + 2],
               imx355_common_data.data[idx + 3]);
}
...
static int open(struct subdrv_ctx *ctx)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == IMX355_SENSOR_ID_23021) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", ctx->i2c_write_id, sensor_id);
                break;
            }
            LOG_INF("Read sensor id fail, id: 0x%x\n", ctx->i2c_write_id);
...
/*LOG_INF("feature_id = %d\n", feature_id);*/
    ...
    case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
        if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
            *(feature_data + 0) =
                sizeof(imx355_ana_gain_table_23021);
        } else {
            memcpy((void *)(uintptr_t) (*(feature_data + 1)),
            (void *)imx355_ana_gain_table_23021,
            sizeof(imx355_ana_gain_table_23021));
        }
        break;
    ...
    *feature_para_len = 4;
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        if(*feature_para_len == sizeof(imx355_common_data) && feature_return_para_32 != feature_para_len) {
            memcpy(feature_return_para_32, &imx355_common_data,
                sizeof(imx355_common_data));
        }
        break;
...
#define EEPROM_8k 0x8000
    case SENSOR_FEATURE_GET_SENSOR_OTP_ALL:
        ...
        imx355_read_eeprom_23021(ctx, 0x00, (BYTE *)feature_return_para_32, *feature_para_len);
    break;
    ...
    case SENSOR_FEATURE_GET_PDAF_DATA:
        ...
        imx355_read_DCC_23021(ctx, (kal_uint16) (*feature_data),
...
static struct subdrv_pw_seq_entry pw_seq[] = {
    {HW_ID_MCLK, 24, 0},
    {HW_ID_RST, 0, 1},
    {HW_ID_AVDD, 2808000, 0},
    {HW_ID_AVDD1, 2808000, 0},
...
const struct subdrv_entry imx355_mipi_raw_23021_entry = {
    .name = "imx355_mipi_raw_23021",
    .id = IMX355_SENSOR_ID_23021,
    .pw_seq = pw_seq,
