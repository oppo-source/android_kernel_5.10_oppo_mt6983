 ...
 *
 ****************************************************************************/
#define PFX "IMX890_pdafotp_23251"
...
static DEFINE_MUTEX(gimx890_eeprom_mutex_23251);

bool read_imx890_eeprom_23251(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size)
{
    if (addr + size > IMX890_MAX_OFFSET_23251) {
        return false;
    }

    if (adaptor_i2c_rd_p8(ctx->i2c_client,
            IMX890_EEPROM_READ_ID_23251 >> 1, addr, data, size) < 0) {
        return false;
    }
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
    ...
    mutex_lock(&gimx890_eeprom_mutex_23251);
    if (pinfo == NULL) {
        read_imx890_eeprom_23251(ctx, 0x1, read_data, 4);
        if (((read_data[3] << 24) |
             (read_data[2] << 16) |
             (read_data[1] << 8) |
    ...
    mutex_unlock(&gimx890_eeprom_mutex_23251);
...
{
    static BYTE IMX890_LRC_data_23251[LRC_SIZE] = { 0 };
    ...
    mutex_lock(&gimx890_eeprom_mutex_23251);
    if ((readed_size == 0) &&
        read_imx890_eeprom_23251(ctx, pinfo->LRC_addr,
                   IMX890_LRC_data_23251, pinfo->LRC_size)) {
        readed_size = pinfo->LRC_size;
    }
    mutex_unlock(&gimx890_eeprom_mutex_23251);

    memcpy(data, IMX890_LRC_data_23251, pinfo->LRC_size);
    return readed_size;
}

unsigned int read_imx890_DCC_23251(struct subdrv_ctx *ctx, BYTE *data)
{
    static BYTE IMX890_DCC_data_23251[DCC_SIZE] = { 0 };
    static unsigned int readed_size;
    struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info(ctx);
    ...
    mutex_lock(&gimx890_eeprom_mutex_23251);
    if ((readed_size == 0) &&
        read_imx890_eeprom_23251(ctx, pinfo->DCC_addr,
                   IMX890_DCC_data_23251, pinfo->DCC_size)) {
        readed_size = pinfo->DCC_size;
    }
    mutex_unlock(&gimx890_eeprom_mutex_23251);

    memcpy(data, IMX890_DCC_data_23251, pinfo->DCC_size);
    return readed_size;
}

struct eeprom_map_info imx890_eeprom_info_23251[] = {
    ...
    { EEPROM_META_STEREO_MW_MAIN_DATA, IMX890_STEREO_MW_START_ADDR_23251, 0x3588, 0x3589, CALI_DATA_MASTER_LENGTH, true },
    { EEPROM_META_STEREO_MT_MAIN_DATA, IMX890_STEREO_MT_START_ADDR_23251, 0x3c24, 0x3c25, CALI_DATA_MASTER_LENGTH, true },
                     ...
                     BYTE *data, int size)
{
    ...
    if (meta_id != imx890_eeprom_info_23251[meta_id].meta)
        return -1;

    if (size != imx890_eeprom_info_23251[meta_id].size)
        return -1;

    addr = imx890_eeprom_info_23251[meta_id].start;
    readsize = imx890_eeprom_info_23251[meta_id].size;

    read_imx890_eeprom_23251(ctx, addr, data, readsize);
