...
#define IMX766DUAL_MAX_OFFSET      0xFFFF
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
	...
	if (iReadRegI2C(pu_send_cmd, 2, (u8 *) data,
			1, IMX766DUAL_EEPROM_READ_ID) < 0) {
		return false;
	}
	return true;
}

static bool read_imx766dual_eeprom(kal_uint16 addr, BYTE *data, int size)
{
	int i = 0;
	int offset = addr;
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(void)
{
	...
	if (pinfo == NULL) {
		read_imx766dual_eeprom(0x1, read_data, 4);
		if (((read_data[3] << 24) |
		     (read_data[2] << 16) |
		     (read_data[1] << 8) |
...
unsigned int read_imx766dual_LRC(BYTE *data)
{
	...
	mutex_lock(&gimx766dual_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx766dual_eeprom(pinfo->LRC_addr,
...
unsigned int read_imx766dual_DCC(BYTE *data)
{
	...
	mutex_lock(&gimx766dual_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx766dual_eeprom(pinfo->DCC_addr,
