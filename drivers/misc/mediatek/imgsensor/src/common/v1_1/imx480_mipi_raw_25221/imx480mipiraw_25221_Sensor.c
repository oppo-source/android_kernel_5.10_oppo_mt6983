...
#include "imx480mipiraw_25221_Sensor.h"

#define DEVICE_VERSION_IMX480    "imx480"
#define LEFT_SIZE (0x4000)
#define READ_EEPROM_1K (1024)
#define MODULE_ID_OFFSET 0x0000
#define DEVICE_VERSION_IMX480_25221     "imx480_25221"
#define IMX480_25221_EEPROM_I2C_ADDR 0xA8
...
static BYTE imx480_25221_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX480_SENSOR_ID_25221,
...
{
    char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2C(pusendcmd, 2, (u8 *)table, size,IMX480_25221_EEPROM_I2C_ADDR );
}
...
static kal_uint16 read_imx480_25221_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, IMX480_25221_EEPROM_I2C_ADDR);

	return get_byte;
}

static void read_imx480_25221_module_data()
{
...
// sensorid : [30 ~ 33]
	...
	memset(imx480_25221_common_data, 0, sizeof(imx480_25221_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		imx480_25221_common_data[8 + idx] = read_imx480_25221_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	...
	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		pr_info("cam data, idx: %02x %02x %02x %02x %d\n",
		       imx480_25221_common_data[idx],
		       imx480_25221_common_data[idx + 1],
		       imx480_25221_common_data[idx + 2],
		       imx480_25221_common_data[idx + 3],
			   idx);
}
        ...
        *sensor_id = return_sensor_id();
        if (*sensor_id == 0x0471) {
            *sensor_id = imgsensor_info.sensor_id;
            pr_info("[get_imgsensor_id] imx480 read sensor id success: 0x%x\n", *sensor_id);
            if(deviceInfo_register_value == 0x00) {
                imgsensor_info.module_id = read_imx480_25221_eeprom_module(MODULE_ID_OFFSET);
                register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_IMX480_25221, imgsensor_info.module_id);
                read_imx480_25221_module_data();
                read_otp_info(otp_data);
                deviceInfo_register_value = 0x01;
            }
            return ERROR_NONE;
        }
        pr_info("[get_imgsensor_id] imx480 read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
        retry--;
    } while (retry > 0);
    i++;
    retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == 0x0471) {
                sensor_id = imgsensor_info.sensor_id;
                pr_info("[open] imx480 Read sensor id success, sensor id: 0x%x\n",sensor_id);
                break;
            }
            pr_info("[open] imx480 Read sensor id fail, i2c_write_id: 0x%x, id: 0x%x\n",imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    if (imgsensor_info.sensor_id != sensor_id) {
        pr_info("imx480 open failed: 0x%x\n", sensor_id);
        return ERROR_SENSOR_CONNECT_FAIL;
    }
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, imx480_25221_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
...
UINT32 IMX480_MIPI_RAW_25221_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
