...
#include "sc201csmipiraw_25221_Sensor.h"
#include "imgsensor_common.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "sc201cs_mipi_25221"
#define LOG_INF(format, args...)    \
	pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
extern void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id);
#define DEVICE_VERSION_C201CS_25221    "sc201cs_25221"
static kal_uint8 deviceInfo_register_value = 0x00;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC201CS_SENSOR_ID_25221,
...
{
	return (((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + SENSOR_ID_OFFSET_25221);
}
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("[get_imgsensor_id] sc201cs Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                if (deviceInfo_register_value == 0x00) {
                    imgsensor_info.module_id = 0x00;
                    register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_C201CS_25221, imgsensor_info.module_id);
                    deviceInfo_register_value = 0x01;
                }
                return ERROR_NONE;
            }
            pr_info("[get_imgsensor_id] sc201cs Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_info("[Open] sc201cs Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                break;
            }
            pr_info("[Open] sc201cs Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SC201CS_MIPI_RAW_25221_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
