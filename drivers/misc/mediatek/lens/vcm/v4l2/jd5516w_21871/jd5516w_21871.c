...
#define JD5516W_NAME				"jd5516w_21871"
...
static int jd5516w_release(struct jd5516w_device *jd5516w)
{
    ...
    val = val > JD5516W_RELEASE_START_CODE ? JD5516W_RELEASE_START_CODE : val;
    jd5516w_set_position(jd5516w, val);
    if (val >= JD5516W_RELEASE_SECOND_CODE) {
        val = JD5516W_RELEASE_SECOND_CODE;
        jd5516w_set_position(jd5516w, val);
        val = JD5516W_RELEASE_THIRD_CODE;
        jd5516w_set_position(jd5516w, val);
    }
    while (val > JD5516W_MIN_CODE) {
        val = val - JD5516W_RELEASE_STEPS;
...
static int jd5516w_init(struct jd5516w_device *jd5516w)
{
    ...
    for (val = JD5516W_INIT_START_CODE;
            val <= JD5516W_INIT_SECOND_CODE;
        val += JD5516W_MOVE_STEPS) {
        ret = jd5516w_set_position(jd5516w, val);
        pr_info("jd5516w set af_reg state is %d set val is %d", ret, val);
        if (ret) {
            pr_info("%s I2C failure: %d",
                __func__, ret);
            return ret;
