...
static inline int AW86017AF_CfgCurrentGears(unsigned char chip_id)
{
	...
	if (chip_id == AW86017_CHIPID) {
		...
		ret = AW86017AF_WriteRegs(AW_REG_IMAX, &gears, AW_DATA_BYTE_1);
		if (ret < 0) {
...
static int AW86017AF_SetPos(unsigned long a_u4Position)
{
	...
	AW_LOGI("Start.");
	while (loop_time < 20) {
		status = AW86017AF_ReadRegs(AW_REG_STATUS, &status_val, AW_DATA_BYTE_1);
...
long AW86017AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int AW86017AF_Release(struct inode *a_pInode, struct file *a_pstFile)
{
	int ret = 0;
	...
	if (*g_pAF_Opened == 2) {
		...
		if (g_motor_type == AW_BOTTOM_MOTOR) {
			...
			while (currPosition > 10) {
				currPosition -= 10;
				ret = AW86017AF_SetPos((unsigned short)currPosition);
				if (ret != AW_SUCCESS)
					AW_LOGE("Set AF target position failed, ret: %d", ret);
				mdelay(1);
			}
...
int AW86017AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int AW86017AF_GetFileName(uint8_t *pFileName)
{
