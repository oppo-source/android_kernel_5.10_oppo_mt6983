...
// static bool ever_dump = 0;
static bool enable_dump = 0;
static struct kernel_param_ops enable_dump_param_ops = {
	.set = param_set_bool,
	.get = param_get_bool,
};

param_check_bool(enable_dump, &enable_dump);
module_param_cb(enable_dump, &enable_dump_param_ops, &enable_dump, 0644);
MODULE_PARM_DESC(enable_dump, "enable_dump");
/**
...
void sched_update_nr_prod(int cpu, unsigned long nr_running, int inc)
{
	...
	if (cpu_over_thres->nr_running_diff < 0 && enable_dump) {
		aee_dump = true;
		printk_deferred("%s: nr_running_diff: %d nr_running:%d nr_over_dn_thres:%d \n",
				TAG, cpu_over_thres->nr_running_diff, nr_running, cpu_over_thres->nr_over_dn_thres);
	}
	...
	if (aee_dump) {
		...
		dump_backtrace(NULL, NULL, KERN_INFO);
		scnprintf(module, sizeof(module), "%s: nr_running_diff: %d nr_running:%d nr_over_dn_thres:%d",
				TAG, cpu_over_thres->nr_running_diff, nr_running, cpu_over_thres->nr_over_dn_thres);
		aee_kernel_exception_api(__FILE__, __LINE__, DB_OPT_DEFAULT|DB_OPT_FTRACE, module, msg);
	}
}
...
static void sched_avg_deferred_func(struct work_struct *work)
{
	...
	over_thresh_chg_notify();
	printk_deferred("core_clt reset \n");
	trace_printk("reset time = %u us\n", div64_u64(sched_clock() - start_time, 1000));
				...
				int policy)
{
	...
	for_each_cpu(cpu, &cls_cpus) {
		...
		if (cpu_over_thres->nr_over_dn_thres < 0 ||
			...
			reset = true;
			printk_deferred("%s: nr_over_dn_thres: %d nr_over_up_thres:%d nr_running_diff:%d",
					TAG, cpu_over_thres->nr_over_dn_thres, cpu_over_thres->nr_over_up_thres, cpu_over_thres->nr_running_diff);
		...
		if (cpu_rq(cpu)->nr_running < cpu_over_thres->nr_over_dn_thres) {
			reset = true;
...
void sched_update_nr_over_thres_prod(struct task_struct *p, int cpu, int over_thres_nr_inc, int idx)
{
	unsigned long flags;
	enum over_thres_type over_type = NO_OVER_THRES;
	bool aee_dump_dn = false;
	bool aee_dump_up = false;
	struct over_thres_stats *cpu_over_thres;
	unsigned long util;
		...
		if (over_type == OVER_UP_THRES) {
			...
			if (diff >= 0) {
				...
				cpu_over_thres->nr_over_dn_thres +=
					over_thres_nr_inc;
				if (cpu_over_thres->nr_over_dn_thres < 0 && enable_dump) {
					aee_dump_dn = true;
					printk_deferred("%s: nr_over_dn_thres: %d pid: %d, comm: %s, idx: %d",
							TAG, cpu_over_thres->nr_over_dn_thres, p->pid, p->comm, idx);				
				}
			}
			...
			if (diff >= 0) {
				...
				cpu_over_thres->nr_over_up_thres +=
					over_thres_nr_inc;
				if (cpu_over_thres->nr_over_up_thres < 0 && enable_dump) {
					aee_dump_up = true;
					printk_deferred("%s: nr_over_up_thres: %d pid: %d, comm: %s, idx: %d",
							TAG ,cpu_over_thres->nr_over_up_thres, p->pid, p->comm, idx);
				}
			}
		} else {/* OVER_DN_THRES */
			diff = (s64) (curr_time -
	...
	if (cpu_over_thres && (aee_dump_dn || aee_dump_up) ){
		char module[100];
		char msg[MAX_MSG_LEN];

		dump_backtrace(NULL, NULL, KERN_INFO);
		scnprintf(module, sizeof(module), "%s: nr_over_dn_thres: %d nr_over_up_thres: %d pid: %d, comm: %s, idx: %d",
				TAG, cpu_over_thres->nr_over_dn_thres, cpu_over_thres->nr_over_up_thres, p->pid, p->comm, idx);
		aee_kernel_exception_api(__FILE__, __LINE__, DB_OPT_DEFAULT|DB_OPT_FTRACE, module, msg);
	}
}
...
void reset_in_fork(void *data, struct task_struct *p)
{
...
void inc_nr_over_thres_running_by_se(void *data, struct sched_entity *se, int flags)
{
	...
	if (se->avg.last_update_time && !(flags & SKIP_AGE_LOAD)) {
		if (entity_is_task(se) && se->on_rq) {
			...
			p = task_of(se);
			count = p->android_vendor_data1[4];
			if (!count) {
				count++;
				p->android_vendor_data1[4] = (u64)count;
			} else {
				printk_deferred("inc_se2: pid=%d comm=%s nr = %d, util_avg = %lu p->on_rq = %d",
						p->pid, p->comm,
						(int)p->android_vendor_data1[4],
						READ_ONCE(p->se.avg.util_avg), p->on_rq);
			}
			sched_update_nr_over_thres_prod(p, cpu_of(task_rq(p)), 1, 4);
		}
	}
}
	...
	if (se->avg.last_update_time && !(flags & SKIP_AGE_LOAD)) {
		...
		if (entity_is_task(se) && se->on_rq) {
			...
			if (count) {
				...
				printk_deferred("dec_se2: pid=%d comm=%s nr = %d, util_avg = %lu p->on_rq = %d",
						p->pid, p->comm,
						(int)p->android_vendor_data1[4],
						READ_ONCE(p->se.avg.util_avg), p->on_rq);
			}
			sched_update_nr_over_thres_prod(p, cpu_of(task_rq(p)), -1, 3);
		}
	}
}
...
{
	int count = p->android_vendor_data1[4];

	if (!count) {
		count++;
		p->android_vendor_data1[4] = (u64)count;
	} else {
		printk_deferred("inc2: pid=%d comm=%s nr = %d, util_avg = %lu p->on_rq = %d",
			p->pid, p->comm,
			(int)p->android_vendor_data1[4],
			READ_ONCE(p->se.avg.util_avg), p->on_rq);
	}

	sched_update_nr_over_thres_prod(p, cpu_of(rq), 1, 1);
}
...
void dec_nr_over_thres_running(void *data, struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (count) {
		...
		printk_deferred("dec2: pid=%d comm=%s nr = %d, util_avg = %lu p->on_rq = %d",
			p->pid, p->comm,
			(int)p->android_vendor_data1[4],
			READ_ONCE(p->se.avg.util_avg), p->on_rq);
	}

	sched_update_nr_over_thres_prod(p, cpu_of(rq), -1, 2);
}
