...
int oplus_get_dts_feature(struct device_node *p_node, char *node_name,
			  char *feature_name, uint32_t *input)
{
	int index = 0;
	int len = 0;
	int ret = 0;
	...
	for (index = 1; index <= 3; index++) {
		...
		ret = of_property_read_string(ch_node, "device_name",
...
{
	int ret = 0;
	ret = of_property_read_u32(ch_node, "use_lb_algo",
				   (u32 *)&g_als_info.use_lb_algo);
...
static void get_accgyro_cali_version(void)
{
	...
	ret = oplus_get_dts_feature(parent_node, "gsensor", "gyro_cali_version",
				    &gyro_cali_version);
	...
	if (ret < 0) {
		...
		DEVINFO_LOG("acc range x y z [%d, %d, %d]", acc_thrd[0], acc_thrd[1],
			    acc_thrd[2]);
...
static ssize_t als_cali_write(struct file *filp, const char *ubuf, size_t cnt,
			      loff_t *data)
{
	char buf[64] = {0};
	void *buf_str = buf;
...
static void oplus_als_cali_data_init(void)
{
	...
	ret = oplus_get_dts_feature(parent_node, "light", "als_ratio" ,
				    &gdata->row_coe);
...
static void get_gold_rear_cct(void)
{
	...
	/*get 3000k gold ch*/
	ret = oplus_get_dts_feature(parent_node, "rear_cct", "gold_rear_cct_3k",
				    gold_rear_cct);
	...
	/*get 6000k gold ch*/
	ret = oplus_get_dts_feature(parent_node, "rear_cct", "gold_rear_cct_6k",
				    gold_rear_cct);
...
static int get_msensor_parameter(struct device_node *ch_node, int num)
{
	...
	if (ret < 0) {
		...
		ret = of_property_read_variable_u32_array(ch_node, "soft-mag-parameter",
				mag_data, 1, 30);
	...
	if (!strcmp(libname, "mmc")
			|| !strcmp(libname, "mxg")) { /*Memsic parameter need analyze*/
		for (index = 0; index < 9; index++) {
			temp_data = mag_data[2 * index];
			sprintf(float_buf, "%c%d.%d%d%d%d", mag_data[2 * index + 1] ? '-' : ' ',
				temp_data / 10000, temp_data % 10000 / 1000, temp_data % 1000 / 100,
				temp_data % 100 / 10,
				temp_data % 10);
...
static ssize_t sensor_feature_write(struct file *filp, const char *ubuf,
