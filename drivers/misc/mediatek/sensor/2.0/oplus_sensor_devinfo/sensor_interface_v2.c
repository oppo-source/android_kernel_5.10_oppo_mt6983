...
static oplus_send_comm_to_hub(int sensor_type, int cmd, void *data,
			      uint8_t length)
{
	int ret = 0;
...
static int oplus_send_factory_mode_cmd_to_hub(int sensor_type, int mode,
		void *result)
{
	return oplus_send_comm_to_hub(sensor_type, OPLUS_ACTION_SET_FACTORY_MODE, &mode,
				      sizeof(mode));
}
...
static int send_utc_time_to_hub(void)
{
	...
	return oplus_send_comm_to_hub(0, OPLUS_ACTION_SCP_SYNC_UTC, utc_data,
				      sizeof(utc_data));
...
{
	uint32_t data = 0;
	DEVINFO_LOG("send lcd info to scp brightness %d, dc_mode %d",
		    (uint32_t)lcd_info->brightness, (uint32_t)lcd_info->dc_mode);
	data = (uint32_t)lcd_info->brightness << 16 | lcd_info->dc_mode;
	return oplus_send_comm_to_hub(SENSOR_TYPE_RGBW, OPLUS_ACTION_SET_LCD_INFO,
				      &data, sizeof(data));
}
