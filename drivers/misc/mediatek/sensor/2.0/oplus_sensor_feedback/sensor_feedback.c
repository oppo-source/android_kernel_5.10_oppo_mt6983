...
static int enable_monitor_sensor(struct sensor_fb_cxt *sensor_fb_cxt)
{
	...
	if (!sensor_fb_cxt->poll_task) {
		sensor_fb_cxt->poll_task = kthread_run(sensor_polling_thread,
						       (void *)sensor_fb_cxt,
	...
	ret = hf_client_custom_cmd(sensor_fb_cxt->client, SENSOR_TYPE_SENSOR_MONITOR,
				   &cust_cmd);
...
{
	struct sensor_fb_cxt *sensor_fb_cxt = container_of(dwork, struct sensor_fb_cxt,
					      enable_sensor_work.work);
...
static int sensor_feedback_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&sensor_fb_cxt->enable_sensor_work,
			  enable_monitor_sensor_work);
	schedule_delayed_work(&sensor_fb_cxt->enable_sensor_work,
