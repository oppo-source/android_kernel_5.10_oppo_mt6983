...
OTP_MAP sc520cs_otp_map_fiji = {
		.module_name = "SC520CS_FIJI",
		.group_num = 3,
		.group_addr_info_num = 2,
		...
		.group_info = {
						{"info",
							{
								{0x80B7, 0x80C8, 0x80B6, 0x80C9},
								{0x80CA, 0x80DB, 0x80B6, 0x80DC},
							},
						},
						{"awb",
							{
								{0x80DE, 0x80E9, 0x80DD, 0x80EA},
								{0x80EB, 0x80F6, 0x80DD, 0x80F7},
							 },
						},
						{"lsc",
							{
								{0x80F9, 0x88AA, 0x80F8, 0x88AB},
								{0x88AC, 0x8FF7, 0x80F8, 0x8FF8},
							},
						},
			},
};
...
static int parse_otp_map_data_sc520cs(OTP_MAP * map, char * data, int temp)
{
	...
	for (i = 0; i < map->group_num; i++) {
		...
		ret = map->readFunc(map->group_info[i].group_addr_info[0].group_flag_addr, &group_flag);
		if (ret < 0) {
			LOG_ERR("read group flag error addr 0x%04x", map->group_info[i].group_addr_info[0].group_flag_addr);
			return -ERROR_I2C;
		}
		LOG_INFO("readFunc: %s flag_addr = 0x%04x, value = 0x%02x\n",
					map->group_info[i].group_name , map->group_info[i].group_addr_info[0].group_flag_addr, group_flag);
		...
		if (!strcmp(map->group_info[i].group_name, "lsc")) {
			if (group_layer == 0) {
				...
				for (addr = map->group_info[i].group_addr_info[group_layer].group_start_addr; addr <=0x87FF; addr++) {
					...
					LOG_DEBUG("Get SC520CS OTP Data of: %s, addr: 0x%04x, data: 0x%02x", map->group_info[i].group_name, addr, *data);
					checksum += *data;
					curr_addr++;
					size++;
					data++;
				...
				for (addr = map->group_info[i].group_addr_info[group_layer].group_start_addr;
					addr <=  map->group_info[i].group_addr_info[group_layer].group_end_addr; addr++) {
					ret = map->readFunc(addr, data);
					if (ret < 0) {
						LOG_ERR(" read data error");
					}
		...
		ret = map->readFunc(map->group_info[i].group_addr_info[group_layer].group_checksum_addr, &readbyte);
		if (checksum == readbyte) {
			curr_addr++;
			size++;
			data++;
...
unsigned int sc520cs_read_region(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	for (i = 0; i < OTP_THREAHOLD; i++) {
		...
		ret = parse_otp_map_data_sc520cs(&sc520cs_otp_map_fiji, &g_otp_buf[IMGSENSOR_SENSOR_IDX_SUB][0], tempotpthreshold);
		if(ret)
			break;
	}
	if (!ret) {
