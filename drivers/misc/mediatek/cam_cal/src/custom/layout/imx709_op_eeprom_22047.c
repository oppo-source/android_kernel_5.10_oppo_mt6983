...
#define PFX "CAM_CAL_IMX709_22047"
...
static unsigned int do_lens_id_imx709(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_imx709(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0157000B, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001300, 0x000005EE, do_pdaf_imx709},
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT imx709_op_eeprom_22047 = {
	.name = "imx709_op_eeprom_22047",
...
#define IMX709_PDAF_PROC1_SIZE  (0x14F0 - 0x1300)  //496
#define IMX709_PDAF_PROC2_SIZE  (0x18EC - 0x1500)  //1004
#define IMX709_PDAF_PROC1_ADDR  (0x1300)
#define IMX709_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_imx709(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = IMX709_PDAF_PROC1_SIZE + IMX709_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX709_PDAF_PROC1_ADDR + IMX709_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX709_PDAF_PROC2_ADDR + IMX709_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX709_PDAF_PROC2_ADDR, IMX709_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[IMX709_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
