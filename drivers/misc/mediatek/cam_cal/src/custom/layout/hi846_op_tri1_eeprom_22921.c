...
unsigned char read_otp_8bit_hi846_tri1(struct i2c_client *client, u16 reg)
{
	int ret;
	u8 buf[2];
...
static void write_otp_8bit_hi846_tri1(struct i2c_client *client, u16 reg, u8 val)
{
	int i4RetValue = 0;
	u8 buf[3];
	struct i2c_msg msg;
...
static void write_otp_16bit_hi846_tri1(struct i2c_client *client, u16 reg, u16 val)
{
	int ret;
	u8 buf[4];
	struct i2c_msg msg;
...
unsigned int Otp_read_region_hi846_tri1(struct i2c_client *client,
	unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	/* 1. sensor init */
	write_otp_16bit_hi846_tri1(client, 0x0A00, 0x0000); //tg_pmem_sckpw/sdly
	write_otp_16bit_hi846_tri1(client, 0x2000, 0x0000); //tg_pmem_sckpw/sdly
	write_otp_16bit_hi846_tri1(client, 0x2002, 0x00FF); //tg_pmem_sckpw/sdly
	write_otp_16bit_hi846_tri1(client, 0x2004, 0x0000); //tg_pmem_rom_dly
	write_otp_16bit_hi846_tri1(client, 0x2008, 0x3FFF); // firmware start address-ROM
	write_otp_16bit_hi846_tri1(client, 0x23FE, 0xC056); // BGR enable
	write_otp_16bit_hi846_tri1(client, 0x0A00, 0x0000); // STRB(OTP Busy) output enable
	write_otp_16bit_hi846_tri1(client, 0x0E04, 0x0012); // STRB(OTP Busy) output drivability
	write_otp_16bit_hi846_tri1(client, 0x0F08, 0x2F04); // Analog PLL setting
	write_otp_16bit_hi846_tri1(client, 0x0F30, 0x001F); // Analog CLKGEN setting
	write_otp_16bit_hi846_tri1(client, 0x0F36, 0x001F); // Analog CLKGEN setting
	write_otp_16bit_hi846_tri1(client, 0x0F04, 0x3A00); // PLL enable
	write_otp_16bit_hi846_tri1(client, 0x0F32, 0x025A); // mipi disable
	write_otp_16bit_hi846_tri1(client, 0x0F38, 0x0256); // TG PMEM CEN anable
	write_otp_16bit_hi846_tri1(client, 0x0F2A, 0x4124); // TG MCU enable
	write_otp_16bit_hi846_tri1(client, 0x006A, 0x0100); // ROM OTP Continuous W/R mode enable
	write_otp_16bit_hi846_tri1(client, 0x004C, 0x0100); // Stream ON

	/* 1. init OTP setting*/
	write_otp_8bit_hi846_tri1(client, 0x0A02, 0x01); //Fast sleep on
	write_otp_8bit_hi846_tri1(client, 0x0A00, 0x00);//stand by on
	mdelay(10);
	write_otp_8bit_hi846_tri1(client, 0x0F02, 0x00);//pll disable
	write_otp_8bit_hi846_tri1(client, 0x071A, 0x01);//CP TRIM_H
	write_otp_8bit_hi846_tri1(client, 0x071B, 0x09);//IPGM TRIM_H
	write_otp_8bit_hi846_tri1(client, 0x0D04, 0x00);//Fsync(OTP busy)Output Enable
	write_otp_8bit_hi846_tri1(client, 0x0D00, 0x07);//Fsync(OTP busy)Output Drivability
	write_otp_8bit_hi846_tri1(client, 0x003E, 0x10);//OTP r/w mode
	write_otp_8bit_hi846_tri1(client, 0x0A00, 0x01);//standby off
	mdelay(1);

	/* 2. read otp data function */
	write_otp_8bit_hi846_tri1(client, 0x070A, (u8)((OTP_addr >> 8) & 0xFF));
	write_otp_8bit_hi846_tri1(client, 0x070B, (u8)(OTP_addr & 0xFF));
	write_otp_8bit_hi846_tri1(client, 0x0702, 0x01);
	for (ret=0; ret < readsize; ret++) {
		//OTP data read
		*(read_data + ret) = read_otp_8bit_hi846_tri1(client, 0x0708);
	}

	/* 3. disable otp function */
	write_otp_8bit_hi846_tri1(client, 0x0a00, 0x00);
	mdelay(10);
	write_otp_8bit_hi846_tri1(client, 0x003e, 0x00);
	write_otp_8bit_hi846_tri1(client, 0x0a00, 0x01);
	mdelay(1);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000201, 0x01051755, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000202, 0x00000001, do_module_version},
		{0x00000001, 0x00000202, 0x00000001, do_part_number},
		{0x00000001, 0x0000022E, 0x0000074C, do_single_lsc_hi846},
		...
		{0x00000001, 0x00000204, 0x00000001, do_lens_id_hi846}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT hi846_op_tri1_eeprom_22921 = {
	.name = "hi846_op_tri1_eeprom_22921",
	.check_layout_function = layout_check,
	.read_function = Otp_read_region_hi846_tri1,
