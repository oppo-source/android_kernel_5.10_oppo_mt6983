...
static bool oplus_wrx_booster_en_is_support(struct oplus_nu1619_ic *chip);
static void nu1619_set_wrx_booster_en_val(int value);
static int nu1619_get_wrx_booster_en_val(void);
...
static bool is_mp2650_chg(void)
{
	int val = 0;
	val = strncmp(oplus_chg_ops_name_get(), "ext-mp2650", 64);
	if (val == 0)
		return true;
	return false;
}
...
static void nu1619_set_rx_charge_current(struct oplus_nu1619_ic *chip, int chg_current)
{
	...
	g_oplus_chip->chg_ops->charging_current_write_fast(chip->nu1619_chg_status.charge_current);
}
...
static void nu1619_charger_init(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP
		...
		if (g_oplus_chip->chg_ops->wls_input_current_write)
...
bool nu1619_onekey_download_firmware(struct oplus_nu1619_ic * chip)
{
	...
	if (oplus_wrx_booster_en_is_support(chip) == true)
		nu1619_set_wrx_booster_en_val(0);
	else
		g_oplus_chip->chg_ops->otg_disable();
	msleep(100);
	if (oplus_wrx_booster_en_is_support(chip) == true)
		nu1619_set_wrx_booster_en_val(1);
	else
		g_oplus_chip->chg_ops->otg_enable();
	msleep(100);
...
static bool oplus_wrx_booster_en_is_support(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->wrx_booster_en_gpio <= 0)
		return false;

	return true;
}

static int nu1619_wrx_booster_en_gpio_init(struct oplus_nu1619_ic *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_nu1619_ic not ready!\n", __func__);
		return -EINVAL;
	...
	chip->wrx_booster_en_active =
			pinctrl_lookup_state(chip->pinctrl, "wrx_booster_en_active");
	if (IS_ERR_OR_NULL(chip->wrx_booster_en_active)) {
		chg_err("get wrx_booster_en_active fail\n");
		return -EINVAL;
	}

	chip->wrx_booster_en_sleep =
			pinctrl_lookup_state(chip->pinctrl, "wrx_booster_en_sleep");
	if (IS_ERR_OR_NULL(chip->wrx_booster_en_sleep)) {
		chg_err("get wrx_booster_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->wrx_booster_en_gpio, 0);
	pinctrl_select_state(chip->pinctrl,
			chip->wrx_booster_en_sleep);

	chg_err("gpio_val:%d\n", gpio_get_value(chip->wrx_booster_en_gpio));
...
static void nu1619_set_wrx_booster_en_val(int value)
{
	...
	if (chip->wrx_booster_en_gpio <= 0) {
		chg_err("wrx_booster_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->pinctrl)
		|| IS_ERR_OR_NULL(chip->wrx_booster_en_active)
		|| IS_ERR_OR_NULL(chip->wrx_booster_en_sleep)) {
		chg_err("pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->wrx_booster_en_gpio, 1);
		pinctrl_select_state(chip->pinctrl,
				chip->wrx_booster_en_active);
	} else {
		gpio_direction_output(chip->wrx_booster_en_gpio, 0);
	...
	chg_err("<~WPC~>set value:%d, gpio_val:%d\n",
		value, gpio_get_value(chip->wrx_booster_en_gpio));
}
...
static int nu1619_get_wrx_booster_en_val(void)
{
	...
	return gpio_get_value(chip->wrx_booster_en_gpio);
}
...
static int nu1619_idt_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->wrx_booster_en_gpio = of_get_named_gpio(node, "qcom,wrx_booster_en-gpio", 0);
	if (chip->wrx_booster_en_gpio < 0) {
		pr_err("chip->wrx_booster_en_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrx_booster_en_gpio)) {
			rc = gpio_request(chip->wrx_booster_en_gpio, "wrx_booster-en-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n", chip->wrx_booster_en_gpio);
			} else {
				rc = nu1619_wrx_booster_en_gpio_init(chip);
				if (rc) {
					chg_err("unable to init wrx_booster_en_gpio:%d\n", chip->wrx_booster_en_gpio);
				}
			}
		}
		pr_err("chip->wrx_booster_en_gpio =%d\n", chip->wrx_booster_en_gpio);
	}
...
EXPORT_SYMBOL(nu1619_driver_init);
