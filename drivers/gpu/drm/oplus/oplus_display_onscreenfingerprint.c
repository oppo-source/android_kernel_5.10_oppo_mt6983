...
static int oplus_ofp_set_panel_hbm(struct drm_crtc *crtc, bool hbm_en)
{
	...
	if (oplus_ofp_local_hbm_is_enabled()) {
		mtk_drm_trace_begin("OPLUS_OFP_SET_LHBM_PRESSED_ICON");
...
/* #ifdef OPLUS_ADFR */
		/* switch to te */
		if (oplus_adfr_is_support() && (oplus_adfr_get_vsync_mode() == OPLUS_EXTERNAL_TE_TP_VSYNC) && !oplus_ofp_full_screen_aod_mode_is_enabled()) {
...
int oplus_ofp_video_mode_aod_handle(void *drm_crtc, void *mtk_panel_ext, void *drm_panel, void *mtk_dsi, void *dcs_write_gce, void *cmdq_handle)
{
	...
	mtk_drm_trace_begin("oplus_ofp_video_mode_aod_handle");
...
int oplus_ofp_notify_fp_press(void *buf)
{
	...
	if (oplus_ofp_is_support() && p_oplus_ofp_params->fp_press && oplus_ofp_video_mode_30hz_aod_is_enabled()) {
		/* send aod off cmd in doze mode to speed up fingerprint unlocking */
...
int oplus_ofp_set_longrui_aod_mode(void *buf)
{
	...
	mtk_drm_trace_c("oplus_ofp_longrui_aod_mode|%d", p_oplus_ofp_params->longrui_aod_mode);
	...
	mtk_drm_trace_begin("oplus_ofp_set_longrui_aod_mode");
...
int oplus_ofp_get_longrui_aod_config(void *buf)
{
	...
	mtk_drm_trace_begin("oplus_ofp_get_longrui_aod_config");
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	mtk_drm_trace_begin("oplus_ofp_set_longrui_aod_mode_attr");
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	mtk_drm_trace_begin("oplus_ofp_get_longrui_aod_config_attr");
