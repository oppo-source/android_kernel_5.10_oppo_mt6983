...
int lcd_set_bl_bias_reg(struct device *pdev, int enable)
{
	...
	if (enable) {
		...
		//write backlight reg
		/* BL_CFG1；OVP=34V，线性调光，PWM enable */
...
static int lcd_bl_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//write backlight reg
	ret = lcd_bl_write_byte(0x02, 0x3B); /* BL_CFG1；OVP=34V，线性调光，PWM enable */
	ret = lcd_bl_write_byte(0x11, 0x37); /* BL_OPTION2；电感4.7uH，BL_CURRENT_LIMIT 2.5A；*/
	ret = lcd_bl_write_byte(0x15, 0xB8); /* Backlight Full-scale LED Current 23.6mA/CH；*/
