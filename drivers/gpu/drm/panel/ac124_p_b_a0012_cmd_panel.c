...
#include "ac124_p_b_a0012_cmd_panel.h"

#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac124_p_b_a0012_data_hw_roundedpattern_l_boe.h"
#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac124_p_b_a0012_data_hw_roundedpattern_r_boe.h"
...
char esd_bl_level[] = {0x51, 0x07, 0xFF};
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	DISP_DEBUG("esd_bl_level[1]=%x, esd_bl_level[2]=%x\n", esd_bl_level[1], esd_bl_level[2]);
...
{
    unsigned int level = oplus_display_brightness;
    char HBM_on_setting[] = {0x51, 0x0F, 0xFF};
    char HBM_off_setting[] = {0x51, 0x00, 0x00};
	...
	if(hbm_mode == 1) {
		cb(dsi, handle, HBM_on_setting, ARRAY_SIZE(HBM_on_setting));
	} else if (hbm_mode == 0) {
        HBM_off_setting[1] = level >> 8;
        HBM_off_setting[2] = level & 0xFF;
        cb(dsi, handle, HBM_off_setting, ARRAY_SIZE(HBM_off_setting));
	    DISP_DEBUG("HBM_off_setting[1]=%x, HBM_off_setting[2]=%x\n", HBM_off_setting[1], HBM_off_setting[2]);
	}
	return 0;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	DISP_DEBUG("%s: p_b_a0012 lcm ctx->prepared %d\n", __func__, ctx->prepared);
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	DISP_INFO("[LCM] %s+ p_b_a0012 Start\n", __func__);
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac124_p_b_a0012_cmd_panel",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("oplus");
MODULE_DESCRIPTION("ac124,p_b,a0012,OLED Driver");
