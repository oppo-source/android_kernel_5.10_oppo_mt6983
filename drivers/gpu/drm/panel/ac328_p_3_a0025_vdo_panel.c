...
#include "ac328_p_3_a0025_vdo_panel.h"
#include "oplus_bl.h"
...
static int get_mode_enum(struct drm_display_mode *m)
{
	...
	if (m == NULL) {
		DISP_ERR("ac328_p_3_a0025 %s display_mode m = NULL!\n", __func__);
		return -EINVAL;
	}
...
{
	DISP_ERR("ac328_p_3_a0025 %s +, mode id=%d\n", __func__, mode_id);
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	DISP_ERR("ac328_p_3_a0025 backlight = %d bl_level[1]=%x, bl_level[2]=%x\n", level, bl_level[1], bl_level[2]);
	...
	if (lhbm_70nit_off_action == 0 && demura_tap != 3) {
		if (mapped_level <= 1147 && demura_tap != 1) {
			demura_tap = 1;
			DISP_ERR("ac328_p_3_a0025 backlight send demura0\n");
			...
			demura_tap = 2;
			DISP_ERR("ac328_p_3_a0025 backlight send demura1\n");
		...
		if (mapped_level <= 1147) {
			demura_tap = 1;
			DISP_ERR("ac328_p_3_a0025 backlight send demura0 , lhbm_70nit_off_action = 1 \n");
			for (i = 0; i < sizeof(dsi_demura0_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura0_bl[i].para_list, dsi_demura0_bl[i].count);
			}
		} else if(mapped_level > 1147) {
			demura_tap = 2;
			DISP_ERR("ac328_p_3_a0025 backlight send demura1 , lhbm_70nit_off_action = 1 \n");
			for (i = 0; i < sizeof(dsi_demura1_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura1_bl[i].para_list, dsi_demura1_bl[i].count);
			}
		}
	...
	DISP_ERR("ac328_p_3_a0025 setbacklight finish\n");
	return 0;
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (lhbm_70nit_off_action == 0 && demura_tap != 3) {
		if (level <= 1147 && demura_tap != 1) {
			demura_tap = 1;
			DISP_ERR("ac328_p_3_a0025 esd send demura_0\n");
			for (i = 0; i < sizeof(dsi_demura0_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura0_bl[i].para_list, dsi_demura0_bl[i].count);
			}
		} else if(level > 1147 && demura_tap != 2){
			demura_tap = 2;
			DISP_ERR("ac328_p_3_a0025 esd send demura_1\n");
			for (i = 0; i < sizeof(dsi_demura1_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura1_bl[i].para_list, dsi_demura1_bl[i].count);
			}
		}
	...
	DISP_ERR("ac328_p_3_a0025 esd_backlight_recovery finish\n");
	return 0;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	if (ret < 0)
		lcm_unprepare(panel);
	DISP_ERR("ac328_p_3_a0025 poweron Successful %s-\n", __func__);
	return 0;
}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	flag_hbm = 0;
	demura_tap = 0;
	DISP_ERR("ac328_p_3_a0025 poweroff Successful %s-\n", __func__);
	return 0;
}
		...
		unsigned int id)
{
	...
	if (*ext_param)
		DISP_DEBUG("ac328_p_3_a0025 data_rate:%d mode_id = %d\n", (*ext_param)->data_rate, mode_id);
	else
		DISP_ERR("ac328_p_3_a0025 ext_param is NULL;\n");
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	register_device_proc("lcd", "AC328_A0025", "P_3");
/* #ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
...
static const struct of_device_id lcm_of_match[] = {
	{
		.compatible = "ac328,p,3,a0025,vdo,panel",
	},
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac328_p_3_a0025_vdo_panel",
		.owner = THIS_MODULE,
