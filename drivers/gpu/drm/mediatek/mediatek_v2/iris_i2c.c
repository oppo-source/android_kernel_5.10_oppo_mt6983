		...
		struct i2c_adapter *adapter, struct i2c_msg *msgs, int len)
{
	...
	for (i = 0; i < len; i++) {
		pos = i << 1;
		ret = i2c_transfer(adapter, &msgs[pos], 1);
		if (ret == 1) {
			ret = 0;
		} else {
			ret = ret < 0 ? ret : -EIO;
			pr_err("%s, %d i2c_transfer failed,write_cmd,ret = %d]\n",
		...
		if (ret == 0) {
			udelay(20);
			ret = i2c_transfer(adapter, &msgs[pos + 1], 1);
			if (ret == 1) {
				ret = 0;
			} else {
				ret = ret < 0 ? ret : -EIO;
				pr_err("%s, %d i2c_transfer failed,read_cmd,ret = %d]\n",
					__func__, __LINE__, ret);
				return ret;
			}
		}
