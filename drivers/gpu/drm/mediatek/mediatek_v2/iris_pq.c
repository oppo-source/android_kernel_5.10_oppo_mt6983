...
static int iris_cm_csc_para_set(struct iris_update_ipopt *popt, uint8_t skip_last, uint32_t csc_ip, uint32_t *csc_value)
{
	...
	if (psopt == NULL) {
		IRIS_LOGE("can not find ip_ = %02x opt_id = %02x", ip, opt_id);
		return 1;
	}
...
void iris_cm_csc_level_set(u32 csc_ip_, u32 *csc_value)
{
	bool skiplast = 0;
	int len;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	...
	iris_init_ipopt_ip(popt,  IP_OPT_MAX);
	iris_capture_disable_pq(popt, &skiplast);
	len = iris_cm_csc_para_set(popt, skiplast, csc_ip_, csc_value);
	len = iris_capture_enable_pq(popt, len);

	is_ulps_enable = iris_disable_ulps(path);
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
	IRIS_LOGD("%s csc len=%d", (csc_ip_ == IRIS_IP_DPP) ? "dpp" : "cm", len);
}
...
static void iris_dynamic_vfr(struct iris_cfg *pcfg)
{
...
void iris_dtg_frame_rate_set(u32 framerate)
{
	...
	use_2nd_tm = iris_belongs_to_2nd_timing(&iris_get_cfg()->timing);
	if (use_2nd_tm)
...
int iris_check_hdr_backlight(u8 *bl, u32 len)
{
	...
	if (len == 1) {
		...
		pr_err("%s bl len is %d", __func__, len);
...
void iris_brightness_level_set(u32 *value)
{
	...
	if (0 /* pcfg && pcfg->osd_enable && pcfg->dynamic_vfr && pcfg->display->panel->is_secondary*/){
		...
		struct iris_cfg *pcfg)
{
	...
	if (IRIS_IF_LOGV()) {
		...
		for (i = 0; i < pinit_val->ip_cnt; i++) {
			...
			snprintf(ptr, sum, "ip_ is %0x opt is ", pval->ip);
			for (j = 0; j < pval->opt_cnt; j++) {
				len = strlen(ptr);
				sum -= len;
