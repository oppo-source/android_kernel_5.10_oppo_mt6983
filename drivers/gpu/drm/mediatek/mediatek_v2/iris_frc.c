...
int32_t iris_set_second_channel_power(bool pwr)
{
	...
	if (pwr) {	//on
		...
		if (0 /*pcfg->panel2->power_info.refcount == 0*/) {
			IRIS_LOGW("%s(), AP mipi2 tx hasn't been power on.", __func__);
			pcfg->osd_switch_on_pending = true;
		} else {
			if (1 /*pcfg->panel2->cur_mode && pcfg->panel2->cur_mode->priv_info &&
...
static void _iris_osd_blending_off(void)
{
	...
	iris_set_pwil_mode(pcfg->pwil_mode, false, DSI_CMD_SET_STATE_HS);
...
static void _iris_osd_blending_on(void)
{
	...
	iris_set_pwil_mode(pcfg->pwil_mode, true, DSI_CMD_SET_STATE_HS);
	//iris_set_pwil_disp_ctrl(); // update disp-ctrl directly
}
