...
static struct iris_cmd_desc *dynamic_lut_send_cmd;
...
static struct iris_cmd_desc *dynamic_lutuvy_send_cmd;
...
int iris_send_lut(u8 lut_type, u8 lut_table_index, u32 lut_abtable_index)
{
	...
	int len = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_cmd_desc *pdesc = pcfg->iris_cmds.iris_cmds_buf;
	...
	case AMBINET_HDR_GAIN:
		...
		memcpy(&pdesc[pcfg->iris_cmds.cmds_index], &dynamic_lutuvy_send_cmd[0],
				sizeof(struct iris_cmd_desc)*iris_lut_param.hdr_lutuvy_pkt_cnt);
...
void iris_update_maxcll_lut(enum LUT_TYPE lutType, u32 lutpos)
{
	...
	if (dynamic_lutuvy_send_cmd == NULL) {
		dynamic_lutuvy_send_cmd = vzalloc(sizeof(struct iris_cmd_desc) * hdr_pkt_size * hdr_block_pkt_cnt * SDR2HDR_LUTUVY_BLOCK_NUMBER);
		if (dynamic_lutuvy_send_cmd == NULL) {
			IRIS_LOGE("%s: failed to alloc mem", __func__);
			return;
		}
