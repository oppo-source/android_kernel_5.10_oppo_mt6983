...
int iris_r_path_select = 0; //0: i2c, 1: dsi
...
void iris_rfb_mode_enter(void)
{
	...
	iris_set_pwil_mode(pcfg->pwil_mode, pcfg->osd_enable, DSI_CMD_SET_STATE_HS);
}
...
static u32 iris_get_frc_vtotal(void)
{
	uint32_t *payload;
	uint32_t vtotal;
	bool use_2nd_tm = iris_belongs_to_2nd_timing(&iris_get_cfg()->timing);
...
void iris_frc_mif_reg_set(void)
{
	...
	u32 vtotal_frcc = iris_get_frc_vtotal();
	...
	u32 input_vtotal = iris_get_frc_vtotal();
	u32 disp_vtotal = iris_get_frc_vtotal();
...
//	u32 pwil_status = iris_ocp_read(IRIS_PWIL_ADDR + PWIL_STATUS, DSI_CMD_SET_STATE_HS);
	u32 pwil_status = 0;
	u32 pwil_mode_state = 0;
	int ret = 0;
	u32 arr[2] = {0};

	arr[0] = IRIS_PWIL_ADDR + PWIL_STATUS;
	ret = iris_i2c_conver_ocp_read(arr, 1, 0);
	if (!ret)
		pwil_status = arr[0];
	pwil_mode_state = pwil_status;
