...
{
	int prj_id = get_project();
	if (24928 == prj_id || 24978 == prj_id || 24979 == prj_id || 24980 == prj_id || 24981 == prj_id)
		return true;
	return false;
}

bool panel_is_baikal_l4(void)
{
	int prj_id = get_project();
	if (prj_id == 25221)
		return true;
...
int mtk_dsi_read_gce_multi(struct mtk_ddp_comp *comp, void *handle,
...
int mtk_dsi_esd_read_v2(struct mtk_ddp_comp *comp, void *handle, void *ptr)
{
    int i;
    struct mtk_dsi *dsi = container_of(comp, struct mtk_dsi, ddp_comp);
    struct mtk_panel_params *params;
    ...
    for (i = 0 ; i < ESD_CHECK_NUM ; i++) {
        ...
        mtk_dsi_read_gce_multi(comp, handle, &t0, &t1, i, ptr);
    }
...
int mtk_dsi_esd_cmp_v2(struct mtk_ddp_comp *comp, void *handle, void *ptr)
{
	int i, j, ret = 0;
	...
	for (i = 0; i < ESD_CHECK_NUM; i++) {
		...
		for (j = 0; j < lcm_esd_tb->count && j < 4; j++) {
			if (lcm_esd_tb->mask_list[j]) {
				DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 before mask lcm_esd_tb->cmd=0x%x,read(0x%x) = expect(0x%x),(0x%x)\n",
			...
			if (chk_val[j] == lcm_esd_tb->para_list[j]) {
				DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 after mask lcm_esd_tb->cmd=0x%x,read(0x%x) = expect(0x%x),(0x%x)\n",
				lcm_esd_tb->cmd,chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				ret = 0;
				if(lcm_esd_tb->revert_flag) {
					ret = -1;
					DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 cmp fail:read(0x%x)=expect(0x%x || 0x%x)\n", chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				}
			} else {
				if(lcm_esd_tb->revert_flag) {
					ret = 0;
					DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 lcm_esd_tb->cmd=0x%x,read(0x%x) = unexpect(0x%x),(0x%x)\n",
							lcm_esd_tb->cmd,chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				} else {
					ret = -1;
					DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 cmp fail:lcm_esd_tb->cmd=0x%x,read(0x%x)!=expect(0x%x),(0x%x)\n",
					lcm_esd_tb->cmd, chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				}
			}
			if (ret < 0) {
				DDPPR_ERR("[DSI]mtk_dsi_esd_cmp_v2 cmp fail:read(0x%x)!=expect(0x%x)\n",
					  ...
					  const void *data, size_t len)
{
	...
	unsigned int is_cmd_mode = mtk_dsi_is_cmd_mode(&dsi->ddp_comp);
	...
	DDPMSG("%s:dsi_en=%d, dsi_mode=%d, is_cmd_mode=%d\n", __func__, dsi->output_en, dsi_mode, is_cmd_mode);
	if (dsi->output_en == true && dsi_mode == 0 && !is_cmd_mode) {
		dsi_mode = 1;
		DDPMSG("%s: force vm cmd for vdo mode\n", __func__);
	}
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	/* Debug info */
	DDPINFO("%s: channel=%d, dsi_mode=%d, flags=0x%x, tx_cmd_num=%d\n",
		__func__, cmd_msg->channel, dsi_mode,
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	/* Debug info */
	DDPINFO("%s: channel=%d, dsi_mode=%d, flags=0x%x, tx_cmd_num=%d, rx_cmd_num=%d\n",
		__func__, cmd_msg->channel, dsi_mode,
		cmd_msg->flags, (int)cmd_msg->tx_cmd_num,
		(int)cmd_msg->rx_cmd_num);
...
//	u32 bpp = mipi_dsi_pixel_format_to_bpp(dsi->format);
	...
	/* DSI BUFFER */
		...
		if (panel_is_aries() || panel_is_bluey()|| panel_is_baikal_l4()) {
			if (en != SET_MMCLK_TYPE_ONLY_CALCULATE)
				mtk_drm_set_mmclk_by_pixclk(&mtk_crtc->base, pixclk, __func__);
		} else {
			mtk_drm_set_mmclk_by_pixclk(&mtk_crtc->base, pixclk, __func__);
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & MODE_DSI_CLK) {
		...
		if (fps_chg_index & MODE_DSI_VFP) {
			...
			DDPMSG("%s:%d, change VFP:%d HFP:%d\n", __func__, __LINE__, vfp, hfp);

			if (dsi && dsi->ext && dsi->ext->params
				&& dsi->mipi_hopping_sta && dsi->ext->params->dyn.vfp) {
				DDPINFO("%s,mipi_clk_change_sta\n", __func__);
...
/* #endif */ /* OPLUS_FEATURE_DISPLAY */
		...
		if (oplus_ofp_video_mode_30hz_aod_is_enabled()) {
			...
			if (dsi && dsi->ext && dsi->ext->params
				&& dsi->mipi_hopping_sta && dsi->ext->params->dyn.hfp) {
				DDPINFO("%s,mipi_clk_change_sta\n", __func__);
				t_hfp = dsi->ext->params->dyn.hfp;
			} else
				t_hfp = hfp;
	...
	if(porch_change_flag & MODE_DSI_HFP) {
		...
		if(!vdo_aod_cfg->in_dsi_init){
			...
			*/
			...
			if (!vdo_aod_cfg->aod_en) {
				dsi->vm.hfront_porch = dst_hfp;
				dsi->vm.vfront_porch = dst_vfp;
			}
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case ESD_CHECK_READ:
		...
		if(panel_ext && panel_ext->params
			&& panel_ext->params->esd_check_multi_v2) {
				mtk_dsi_esd_read_v2(comp, handle, params);
		} else {
	...
	case ESD_CHECK_CMP:
		...
		if (panel_ext && panel_ext->funcs
			...
			&& panel_ext->params->esd_check_multi_v2) {
			return mtk_dsi_esd_cmp_v2(comp, handle, params);
		} else {
