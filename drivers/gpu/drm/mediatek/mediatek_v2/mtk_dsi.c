	...
	struct mtk_drm_crtc *mtk_crtc, unsigned int en)
{
	...
	unsigned int bubble_rate_forcmd = 110;
	unsigned int data_rate;
...
//	u32 bpp = mipi_dsi_pixel_format_to_bpp(dsi->format);
	...
	if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
		...
		u32 line_time = 0, send_date_time = 1, ps_wc;

		ps_wc = (((ext->params->dsc_params.chunk_size + 2) / 3) * 3);
		if (ext->params->dsc_params.slice_mode == 1)
			 ps_wc *= 2;

		send_date_time = ps_wc / dsi->lanes + 1;
		line_time = (4 + 105 + 2 + 5 + ps_wc + 2) / dsi->lanes + 1;
		...
		pixclk = pixclk * bubble_rate_forcmd / 100;
		pixclk = pixclk * (send_date_time * 100 / line_time) / 100;

		DDPMSG("line_time =%d, send_date_time=%d\n", line_time, send_date_time);
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_BL:
	{
		...
		if (panel_ext && panel_ext->funcs
			...
			DISP_DEBUG("DSI_SET_BL: hbm_en=%d, backlight=%d\n", hbm_en, backlight);
