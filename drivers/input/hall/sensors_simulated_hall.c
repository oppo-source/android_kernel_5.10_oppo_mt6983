...
struct hall_simulated_data *g_hall_data1 = NULL;
struct hall_simulated_data *g_hall_data2 = NULL;
...
static void notify_delay_work_func(struct work_struct *work)
{
	int val = 0;

	if (NULL == g_hall_data1 || NULL == g_hall_data2) {
		MHALL_ERR("%g_hall_data is null \n");
		return;
	}

	val = gpio_get_value(g_hall_data1->irq_gpio);
	MHALL_ERR("%d--wireless_ic_probe_done , gpio %d %d\n", g_hall_data1->id, g_hall_data1->irq_gpio, val);
	if (!val) {
		blocking_notifier_call_chain(&hall_notifier, 1, NULL);
		MHALL_ERR("%d--wireless_ic_probe_done , gpio %d %d\n", g_hall_data1->id, g_hall_data1->irq_gpio, val);
		return;
	}
	val = gpio_get_value(g_hall_data2->irq_gpio);
	MHALL_ERR("%d--wireless_ic_probe_done , gpio %d %d\n", g_hall_data2->id, g_hall_data2->irq_gpio, val);
	if (!val) {
		blocking_notifier_call_chain(&hall_notifier, 1, NULL);
		MHALL_ERR("%d--wireless_ic_probe_done , gpio %d %d\n", g_hall_data2->id, g_hall_data2->irq_gpio, val);
	}
...
{
	schedule_delayed_work(&delay_work, msecs_to_jiffies(WORK_DELAY_VALUE));
...
static int hall_simulated_driver_probe(struct platform_device *dev)
{
	...
	if (hall_data->id == 1)
		g_hall_data1 = hall_data;
	else
		g_hall_data2 = hall_data;

	INIT_DELAYED_WORK(&delay_work, notify_delay_work_func);
...
MODULE_DEVICE_TABLE(of, hall_sensor_match_table);
...
//arch_initcall(simulated_hall_init);
