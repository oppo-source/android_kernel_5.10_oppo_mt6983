...
void ufssid_init(struct ufsf_feature *ufsf)
{
	...
	int ret;
	struct ufs_dev_info *dev_info = &(ufsf->hba->dev_info);
	char *product_name = dev_info->model;

	if (product_name[8] == 'G') {
		ERR_MSG("SID %s(), solvit device controller detected! SID enabled.", __func__);
	} else {
		ERR_MSG("SID %s(), %s is not solvit device controller, skip SID.", __func__, product_name);
		return;
	}
	...
	ret = ufsf_query_flag_retry(ufsf->hba, UPIU_QUERY_OPCODE_SET_FLAG,
				    QUERY_FLAG_IDN_STREAM_ID_EN, 0,
				    UFSFEATURE_SELECTOR,
				    &sid->stream_id_enabled);
	ERR_MSG("SID %s(), SET QUERY_FLAG_IDN_STREAM_ID_EN, ret=%s\n", __func__, ret ? "FAIL":"OK");
	...
	ERR_MSG("UFS SID create sysfs finished");
}
