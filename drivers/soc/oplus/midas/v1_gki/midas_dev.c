...
static int rasm_resume(struct device *dev)
{
	...
	data->last_resume_millsec_time = resume_time.tv_sec * 1000 +
					 resume_time.tv_usec / 1000;
	...
	rasm_env[index++] = kasprintf(GFP_KERNEL, "suspend=%ld",
...
static int rasm_suspend(struct device *dev)
{
	...
	data->last_suspend_millsec_time = suspend_time.tv_sec * 1000 +
					  suspend_time.tv_usec / 1000;
...
static int midas_dev_open(struct inode *inode, struct file *filp)
{
	...
	struct midas_priv_info *info = kzalloc(sizeof(struct midas_priv_info),
					       GFP_KERNEL);
...
int __init midas_dev_init(void)
{
	...
	rc = register_trace_android_vh_cpufreq_acct_update_power(
...
err_device_register:
	unregister_trace_android_vh_cpufreq_acct_update_power(midas_record_task_times,
			NULL);
err_trace_register:
	return rc;
}
