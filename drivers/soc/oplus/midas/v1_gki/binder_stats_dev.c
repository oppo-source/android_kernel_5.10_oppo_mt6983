...
static inline bool hash_match(struct binder_notify *bn,
			      struct binder_stats_item *stats_item, int enable_binder_comm)
{
	...
	if (1 == enable_binder_comm)
		return (0 == strncmp(stats_item->caller_comm, bn->caller_task->comm,
				     TASK_COMM_LEN)
			&& 0 == strncmp(stats_item->caller_proc_comm, caller_proc_task->comm,
					TASK_COMM_LEN)
			...
			&& 0 == strncmp(stats_item->binder_proc_comm, binder_proc_task->comm,
					TASK_COMM_LEN));
				...
				struct binder_notify *bn)
{
	...
	binder_proc_task = NULL == bn->binder_task->group_leader ? bn->binder_task :
			   bn->binder_task->group_leader;
	binder_uid = from_kuid_munged(current_user_ns(), task_uid(bn->binder_task));

	if (!context_ptr->has_intr_srv_name && !context_ptr->has_intr_proc_comm
			&& !context_ptr->has_intr_uid)
		intreresting = true;

	else
		intreresting = false;

	hash_for_each_possible(context_ptr->intre_srv_name_hash, hash_node_srv_name,
			       hentry,
	...
	hash_for_each_possible(context_ptr->intre_proc_comm_hash, hash_node_proc_comm,
			       hentry,
	...
	hash_for_each_possible(context_ptr->intre_uid_hash, hash_node_uid, hentry,
			       (long long)binder_uid) {
...
static void store_binder_stats_to_kernel(struct binder_notify *data)
{
	...
	list_for_each_entry(context_ptr, &g_binder_stats_driver.user_list_head,
		...
		hash_for_each_possible(context_ptr->kernel_binder_stats_hash, ref_hash_node,
				       hentry, hash_key(data, context_ptr->enable_binder_comm)) {
		...
		if (NULL != find_item)
			...
			if (kernel_binder_stats->valid_item_cnt + 1 <=
				...
				if (NULL != caller_task->group_leader)
					strncpy(find_item->caller_proc_comm, caller_task->group_leader->comm,
						TASK_COMM_LEN);
				...
				find_item->caller_uid = from_kuid_munged(current_user_ns(),
							task_uid(caller_task));
				/* caller comm */
				strncpy(find_item->caller_comm, caller_task->comm, TASK_COMM_LEN);

				/* service name */
				strncpy(find_item->service_name, data->service_name,
					OPLUS_MAX_SERVICE_NAME_LEN);

				/* binder proc comm */
				if (NULL != binder_task->group_leader)
					strncpy(find_item->binder_proc_comm, binder_task->group_leader->comm,
						TASK_COMM_LEN);
				...
				find_item->binder_uid = from_kuid_munged(current_user_ns(),
				...
				ref_hash_node = &
						(context_ptr->kernel_binder_stats_refs[kernel_binder_stats->valid_item_cnt]);
				ref_hash_node->item = find_item;
				hash_add(context_ptr->kernel_binder_stats_hash, &ref_hash_node->hentry,
...
static void binder_stats_clear_user_context(
	struct binder_stats_user_context *context_ptr)
{
	...
	hash_for_each_safe(context_ptr->kernel_binder_stats_hash, i, tmp, ref_hash_node,
			   hentry) {
...
static int user_enable_binder_stats(
	struct binder_stats_user_context *context_ptr,
	int enable)
{
	unsigned long flags;
	...
	if (1 == enable) {
		...
		binder_stats_buffer_size = 2 * sizeof(unsigned int) + max_item_cnt * sizeof(
						   struct binder_stats_item);
		...
		context_ptr->kernel_binder_stats_refs = vmalloc_user(max_item_cnt * sizeof(
				struct binder_stats_item_ref));
...
static int user_update_binder_stats(
	struct binder_stats_user_context *context_ptr)
{
	int ret = 0;
	unsigned long flags;
...
static int user_binder_stats_cfg_clear(
	struct binder_stats_user_context *context_ptr)
{
	struct binder_stats_filter_srv_name_node *hash_node_srv_name;
	struct binder_stats_filter_proc_comm_node *hash_node_proc_comm;
	...
	hash_for_each_safe(context_ptr->intre_srv_name_hash, i, tmp, hash_node_srv_name,
			   hentry) {
	...
	struct binder_stats_filter_srv_name *filter_srv_name)
{
	...
	hash_node_srv_name = vmalloc_user(sizeof(struct
					  binder_stats_filter_srv_name_node));
	...
	struct binder_stats_filter_proc_comm *filter_proc_comm)
{
	...
	hash_node_proc_comm = vmalloc_user(sizeof(struct
					   binder_stats_filter_proc_comm_node));
	...
	struct binder_stats_handle_name *handle_name)
{
	...
	handle_name_new = kmalloc(sizeof(struct binder_stats_handle_name_node),
				  GFP_ATOMIC);
	...
	hash_for_each_possible(g_binder_stats_driver.handle_name_hash, handle_name_tmp,
			       hentry_handle, handle_name->handle) {
		if (handle_name_tmp->handle == handle_name->handle)
			handle_name_find = handle_name_tmp;
	}

	if (NULL != handle_name_find) {
		strncpy(handle_name_find->service_name, handle_name->service_name,
			OPLUS_MAX_SERVICE_NAME_LEN);
		handle_name_find->service_name[OPLUS_MAX_SERVICE_NAME_LEN - 1] = '\0';

	} else {
		handle_name_new->handle = handle_name->handle;
		strncpy(handle_name_new->service_name, handle_name->service_name,
			OPLUS_MAX_SERVICE_NAME_LEN);
		handle_name_new->service_name[OPLUS_MAX_SERVICE_NAME_LEN - 1] = '\0';
		hash_add(g_binder_stats_driver.handle_name_hash,
			 &handle_name_new->hentry_handle, handle_name_new->handle);
	}
...
static int binder_stats_driver_release(struct inode *ignored,
				       struct file *filp)
{
	struct binder_stats_user_context *context_ptr;
	unsigned long flags;
				      ...
				      unsigned long arg)
{
	...
	case BINDER_STATS_CTL_CFG_ENABLE_BINDER_COMM: {
		...
		if (0 == user_binder_stats_cfg_set_enable_binder_comm(context_ptr,
				enable_binder_comm))
			ret = BINDER_STATS_CTL_RET_SUCC;
	...
	case BINDER_STATS_CTL_CFG_SVR_FILTER_NAME: {
		if (0 != copy_from_user(&filter_srv_name, (char *)arg,
		...
		if (0 == user_binder_stats_add_intreresting_svr_name(context_ptr,
				&filter_srv_name))
			ret = BINDER_STATS_CTL_RET_SUCC;
	...
	case BINDER_STATS_CTL_CFG_SVR_FILTER_PROC_COMM: {
		if (0 != copy_from_user(&filter_proc_comm, (char *)arg,
		...
		if (0 == user_binder_stats_add_intreresting_proc_comm(context_ptr,
				&filter_proc_comm))
			ret = BINDER_STATS_CTL_RET_SUCC;
	...
	case BINDER_STATS_CTL_CFG_SVR_FILTER_UID: {
		if (0 != copy_from_user(&filter_uid, (int *)arg,
	...
	case BINDER_STATS_CTL_SRVMGR_SET_HANDLE_NAME: {
		if (0 != copy_from_user(&handle_name, (int *)arg,
					sizeof(struct binder_stats_handle_name))) {
...
void binder_proc_transaction_hook(void *data,
				  struct task_struct *caller_task, struct task_struct *binder_proc_task,
				  ...
				  int node_debug_id, unsigned int code, bool pending_async)
{
	...
	if (g_binder_stats_driver.srvmgr_tgid == task_tgid_nr(binder_proc_task)) {
		...
		spin_lock_irqsave(&g_binder_stats_driver.srv_name_lock, flags);
		handle_name_find = NULL;
		hash_for_each_possible(g_binder_stats_driver.debug_id_name_hash,
				       handle_name_tmp, hentry_debug_id, node_debug_id) {
			if (handle_name_tmp->node_debug_id == node_debug_id)
				handle_name_find = handle_name_tmp;
		}

		if (NULL != handle_name_find) {
			strncpy(bn.service_name, handle_name_find->service_name,
				OPLUS_MAX_SERVICE_NAME_LEN);
	...
	if (stats_flag) {
		...
		if (NULL != binder_th_task) {
			BINDER_STATS_LOGI("BDS_TRAN %d(%s) -> %d(%s) %d(%s) %d(%s)\n", caller_task->pid,
					  ...
					  binder_proc_task->pid, binder_proc_task->comm, binder_th_task->pid,
					  binder_th_task->comm);
			 ...
			 struct task_struct *proc, uint32_t ref_desc, int node_debug_id)
{
	...
	if (g_binder_stats_driver.srvmgr_tgid < 0
	...
	if (NULL == handle_name_find) {
		...
		hash_add(g_binder_stats_driver.handle_name_hash,
			 &handle_name_new->hentry_handle, ref_desc);
		hash_add(g_binder_stats_driver.debug_id_name_hash,
			 &handle_name_new->hentry_debug_id, node_debug_id);
	} else
			   ...
			   size_t count, loff_t *ppos)
{
	...
	if (strncmp(cmd_buffer, "srvmap", 6) == 0) {
		...
		hash_for_each_safe(g_binder_stats_driver.handle_name_hash, i, tmp,
				   handle_name_find, hentry_handle) {
			BINDER_STATS_LOGI("BDS_DBG service_name:%s handle:%d node_debug_id:%d\n",
					  handle_name_find->service_name, handle_name_find->handle,
					  handle_name_find->node_debug_id);
		}
	}
...
int __init binder_stats_dev_init(void)
{
	...
	g_binder_stats_driver.dev_class = class_create(THIS_MODULE,
					  "binder_stats_class");
	...
	register_trace_android_vh_binder_proc_transaction(binder_proc_transaction_hook,
			NULL);
...
#if defined(BINDER_STATS_DEBUGFS)
	g_binder_stats_driver.d_folder_binder_stats = debugfs_create_dir("binder_stats",
			NULL);
...
#endif
	unregister_trace_android_vh_binder_proc_transaction(
		binder_proc_transaction_hook, NULL);
