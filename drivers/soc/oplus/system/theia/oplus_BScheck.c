...
static ssize_t black_screen_check_proc_write(struct file *file,
		...
		size_t count, loff_t *off)
{
	...
	def_swich = tmp_status == BLACK_STATUS_CHECK_ENABLE
		    || BLACK_STATUS_CHECK_DEBUG == tmp_status;
...
static ssize_t black_screen_report_proc_write(struct file *file,
		const char __user *buf,
		size_t count, loff_t *off)
{
...
static ssize_t black_screen_report_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
...
static ssize_t black_screen_switch_proc_write(struct file *file,
		const char __user *buf,
		size_t count, loff_t *off)
{
	int tmp_stage = 0;
...
static ssize_t black_screen_switch_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
	return 0;
...
static ssize_t black_screen_check_status_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
	char page[256] = {0};
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
static int black_mark_or_clear_error_happened_flag(struct black_data *bla_data,
		int mark_or_clear)
{
	struct file *fp;
	loff_t pos;
...
static int black_write_error_count(struct black_data *bla_data)
{
	...
	if (have_read_old == false) {
		...
		bla_data->error_count_new = bla_data->error_count_new +
					    bla_data->error_count_old;
		have_read_old = true;
...
static void black_determine_init_status_work(struct work_struct *work)
{
	...
	if (bla_data->status != BLACK_STATUS_CHECK_ENABLE
			&& bla_data->status != BLACK_STATUS_CHECK_DEBUG) {
...
static int __init black_screen_check_init(void)
{
	...
	proc_create("blackCheckStatus", S_IRWXUGO, NULL,
	...
	INIT_WORK(&g_black_data.clear_error_happened_flag_work,
		  black_clear_error_happened_flag_work);
	INIT_DELAYED_WORK(&g_black_data.determine_init_status_work,
			  black_determine_init_status_work);
