...
static ssize_t bright_screen_check_proc_read(struct file *file,
		char __user *buf,
...
static ssize_t bright_screen_check_proc_write(struct file *file,
		...
		size_t count, loff_t *off)
{
	...
	ret = sscanf(buffer, "%d %u %d %d", &tmp_status, &tmp_timeout, &tmp_isPanic,
		     &tmp_log);
	...
	def_swich = tmp_status == BRIGHT_STATUS_CHECK_ENABLE
		    || BRIGHT_STATUS_CHECK_DEBUG == tmp_status;
	BRIGHT_DEBUG_PRINTK("%s: status = %d timeout_ms = %u is_panic = %d\n",
			    __func__, g_bright_data.status, g_bright_data.timeout_ms,
			    g_bright_data.is_panic);
...
static ssize_t bright_screen_check_status_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
	char page[256] = {0};
...
static int bright_mark_or_clear_error_happened_flag(struct bright_data
		*bri_data, int mark_or_clear)
{
	struct file *fp;
	loff_t pos;
	ssize_t len = 0;
...
static int bright_write_error_count(struct bright_data *bri_data)
{
	...
	if (have_read_old == false) {
		...
		bri_data->error_count_new = bri_data->error_count_new +
					    bri_data->error_count_old;
		have_read_old = true;
...
static void bright_determine_init_status_work(struct work_struct *work)
{
	...
	if (bri_data->status != BRIGHT_STATUS_CHECK_ENABLE
			&& bri_data->status != BRIGHT_STATUS_CHECK_DEBUG) {
...
static ssize_t bright_screen_switch_proc_write(struct file *file,
		const char __user *buf,
		size_t count, loff_t *off)
{
	int tmp_stage = 0;
...
static ssize_t bright_screen_switch_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
	return 0;
...
static int __init bright_screen_check_init(void)
{
	...
	proc_create("brightCheckStatus", S_IRWXUGO, NULL,
		    &bright_screen_check_status_proc_fops);
	...
	INIT_WORK(&g_bright_data.clear_error_happened_flag_work,
		  bright_clear_error_happened_flag_work);
	INIT_DELAYED_WORK(&g_bright_data.determine_init_status_work,
			  bright_determine_init_status_work);
