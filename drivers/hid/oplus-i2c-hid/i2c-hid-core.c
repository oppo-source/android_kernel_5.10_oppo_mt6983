...
static ssize_t proc_kbd_keypad_status_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[PROC_PAGE_LEN] = {0};

	snprintf(page, PROC_PAGE_LEN - 1, "%u", kb_connected ? 1 : 0);
...
static const struct proc_ops proc_kbd_keypad_status_ops = {
	.proc_read = proc_kbd_keypad_status_read,
};

static ssize_t proc_kbd_touch_disable_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	struct i2c_hid *ihid = PDE_DATA(file_inode(file));
	uint8_t ret = 0;
	char page[PROC_PAGE_LEN] = {0};
	...
	snprintf(page, PROC_PAGE_LEN - 1, "%d\n", ihid->keyboard_touch_disable);
...
static ssize_t proc_kbd_touch_disable_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	char write_data[PROC_PAGE_LEN] = { 0 };
	struct i2c_hid *ihid = PDE_DATA(file_inode(file));
	...
	if (write_data[0] == '1') {
		ihid->keyboard_touch_disable = true;
		Touchmcu_disable_report(ihid);
		i2c_hid_err(ihid, "%s: Enter keyboard touch disable.\n", __func__);
	} else {
		ihid->keyboard_touch_disable = false;
		Touchmcu_enable_report(ihid);
		i2c_hid_err(ihid, "%s: Exit keyboard touch disable.\n", __func__);
	}

	return count;
}

static const struct proc_ops proc_kbd_touch_disable_ops = {
	.proc_read = proc_kbd_touch_disable_read,
	.proc_write = proc_kbd_touch_disable_write,
};
...
static void init_i2c_hid_proc(struct i2c_hid *ihid)
{
	...
	if(ihid->pogopin_fw_support) {
		...
		prEntry_tmp = proc_create_data("kbd_keypad_status", 0444, prEntry_pogopin, &proc_kbd_keypad_status_ops, ihid);
		if (prEntry_tmp == NULL) {
			i2c_hid_err(ihid, "%s: create kbd_keypad_status proc entry failed.\n", __func__);
		}

		prEntry_tmp = proc_create_data("kbd_touch_disable", 0664, prEntry_pogopin, &proc_kbd_touch_disable_ops, ihid);
		if (prEntry_tmp == NULL) {
			i2c_hid_err(ihid, "%s: create kbd_touch_disable proc entry failed.\n", __func__);
		}
	}
