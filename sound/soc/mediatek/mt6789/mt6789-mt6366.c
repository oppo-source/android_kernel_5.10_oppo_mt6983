...
static int mt6789_mt6366_dev_probe(struct platform_device *pdev)
{
	...
	/* 2024/12/17, add for 4 Analog PA control */
	ret = of_property_read_u32(pdev->dev.of_node,
			"oplus,oplus-analog-4pa", &oplus_analog_4pa);
	if (ret) {
		dev_info(&pdev->dev, "%s() read prop oplus-analog-4pa fail\n", __func__);
		need_analog_4pa = false;
	} else {
		dev_info(&pdev->dev, "%s() read prop oplus-analog-4pa %d\n", __func__, oplus_analog_4pa);
		need_analog_4pa = oplus_analog_4pa;
	}
#endif
