...
void oplus_root_check_succ(uid_t uid, uid_t euid, uid_t egid, uid_t callnum)
{
	...
	dcs_event->payload_length = snprintf(dcs_event_payload, 256,
					     "%d$$old_euid@@%d$$old_fsuid@@%d$$sys_call_number@@%d$$addr_limit@@"\
					     "%lx$$curr_uid@@%d$$curr_euid@@%d$$curr_fsuid@@%d$$curr_name@@%s$$ppid@@%d$$ppidname@@%s$$enforce@@%d\n",
...
#else
	dcs_event->payload_length = snprintf(dcs_event_payload, 256,
					     "%d$$old_euid@@%d$$old_egid@@%d$$sys_call_number@@%d$$addr_limit@@"\
					     "%lx$$curr_uid@@%d$$curr_euid@@%d$$curr_egid@@%d$$curr_name@@%s$$ppid@@%d$$ppidname@@%s$$enforce@@%d\n",
					     uid, euid, egid, callnum,
